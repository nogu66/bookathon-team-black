...
ここで、私と似た属性の人だけでなく、@<b>{仕事や趣味でのエンジニアリング活動で何らかの課題を抱えている人}にも、Bookathonは効果があるのではないかと考えました。私のやり方をそのまま真似するのではなく、各々が抱えている課題に対して、以下のような流れで取り組むのが理想的だと思います。

 1. 自分が抱えている課題を明確にする
 2. Bookathon中に何をテーマに書けば、その課題の解消に繋がるか？の仮説を立てる
 3. メンターさんからアドバイスをもらう
 4. アドバイスと自分の仮説を見比べることで、自分が今まで見えていなかった気づきを得る
 5. より深い気づきを得て、課題への理解が深まる

この構造のポイントは、@<b>{自分で仮説を立てた上でメンターのアドバイスを聞く}ことです。仮説なしにアドバイスを聞くのと、仮説を持った上で聞くのとでは、得られる気づきの深さが全く違います。具体例として、次の節で私自身の仮説の変遷を紹介します。

=== 私の応募前の仮説

私は応募前に、以下のような仮説を立てていました。

 * @<b>{課題}：仕事が遅い・実力不足と感じる
 * @<b>{原因の仮説}：基礎がなってないから。なぜ基礎がなってないかというと、基礎学習に面白さを見出せず疎かにしているから
 * @<b>{解決策の仮説}：Bookathonという新しいイベントで、強制的にたくさんのインプット/アウトプットを行う企画にしてしまえば、楽しめるし頑張れるのではないか

課題と原因は正直に自覚していたつもりでした。解決策としては、「面白くないなら、面白い環境に身を置けばいい」というシンプルな発想です。しかし、この仮説にはメンターのwakameさんから重要なフィードバックをもらうことになります。
...
wakameさんのアドバイスを受けて、応募前に立てた仮説を改めて振り返ってみました。課題の認識はあっていましたが、@<b>{原因の深掘りが足りていなかった}ことに気づきました。

 * @<b>{原因の深掘り}：なぜ基礎学習に面白さを感じないのか？それは、基礎学習として@<b>{受動的に情報を受け取るもの}しか知らなかったからです。本を読んだり、チュートリアルに書かれた内容を写経したり。今回のように、問いを持ってそれに関して仮説を立てて検証するという@<b>{能動的な学習}は、面白いと感じました
 * @<b>{解決策の見直し}：自分は、困ったらパワー系の解決策に走ってしまい、実行しきれずに頓挫しがちです。「仮説を立てて、実験して検証する」という行為はコスパが悪いものだと思っていました。実際にやってみて、確かに時間はかかりますが、得るものが大きかったです

この気づきを踏まえて、私の方針が固まりました。今回は、1つの仮説を手を動かして検証することにします。使う技術はTypeScriptに絞ることにしました。
...
まずは自分で考えてみて、煮詰まったらAIに聞くようにします。最初からAIに聞いてしまうと、問いを立てる力や、仮説を立てる力が付かないと思うからです。実際にやってみて気づいたのですが、@<b>{AIを使わずに問いを立てて、問いの答えについて仮説を立てようとすると、自分が何もわかっていないことがわかります。}「わかっていないこと」について、「わかっていなかった」と気づけた。これだけでも大きな収穫でした。

==== ② 執筆時、ありのままを書くことを恥ずかしがらない

「こんな初歩的なことも知らないことがバレるのは恥ずかしい...」と自分を偽らないようにします。恥ずかしい気持ちをあえて感じることで、これからの勉強への原動力にするためです。

==== ③ 考えることにストレスを感じたら、AIに頼る

①で「最初からAIに聞かない」と書きましたが、考え続けてストレスになるまで粘る必要はありません。煮詰まったら、素直にAIに頼ることも大切です。大事なのは、まず自分で考える時間を持つこと。その上でAIを使えば、AIの回答を自分の仮説と比較できるので、理解が深まります。
...
ここからの検証パートは、筆者個人のアウトプットを含んでおり、読者の方にとって直接役立つ内容ではないかもしれません。アウトプットの内容そのものというよりは、「こんな流れで仮説を立てて検証を進めているんだな」という雰囲気を掴んでもらえたら嬉しいです。読み飛ばしたい方は、「Bookathonハックで得た学び」の章まで飛んでいただければと思います。
//}
...
「全部置換すればいいんじゃないか？」という疑問を持ったので、実際に検証してみることにしました。AIに「コード追加は最小限で、なおかつ『置き換えればいいじゃん』が通用しないことを私に分からせるケースを作ってください」と指示して、コードを追加してもらいました。以下のコードは、Axiosの@<code>{response.data}を@<code>{await response.json()}に@<b>{単純置換した結果、エラーが起きることを示すための実験コード}です。意図的にエラーが出るように書いています。
...
特に印象的だったのは、@<b>{AIを使わずに問いを立てて、仮説を立てようとするプロセスそのものが、自分の無知を映す鏡になった}ということです。型エラーについて考え始めたとき、「案外知らないこと多い！！」と驚きましたが、あれこそがまさに仮説駆動学習の効果でした。AIに最初から聞いていたら、「ふーん、そうなんだ」で終わっていたはずです。自分で考えたからこそ、「わかっていないこと」に気づけました。
...
Bookathonを経験して、他の学び方との違いが見えてきました。

==== ハッカソンとの違い

 * @<b>{自分のペースで学習できる}：ハッカソンはチームの進捗に合わせる必要があるが、Bookathonは自分の課題に集中できる
 * @<b>{自分自身の課題に向き合える}：ハッカソンは「何を作るか」が主題だが、Bookathonは「自分が何を理解していないか」に向き合える
 * @<b>{アウトプットの形が言語化}：言語化するからこそ、理解や考えが甘い部分が如実に出る。ハッカソンは、それっぽいものが作れれば誤魔化せる時もある

==== 家での学習との違い

 * @<b>{強制力がある}：締め切りと周りの参加者がいることで、サボれない環境ができる
 * @<b>{豪華かつすぐ聞けるメンターがいる}：家で一人で学習していたら出会えないレベルのメンターに、その場で質問できる

==== 会社でメンターに教えてもらうこととの違い

日頃の業務や1on1でも、変えた方が良い点に気づく機会はあります。しかし、@<b>{それをすぐに強制的に実行できるわけではありません。}私のような人間は、その時は「よしやろう！」という気分になっても、実行できる機会には気持ちが半減していたりします。Bookathonは、気づいたことをその場で即座に試せる環境です。

==== Bookathonのデメリット

一方で、執筆活動のコストは大きいです。学習だけに集中したい人には向かないかもしれません。

==== 技術書執筆への門戸が開かれる

これは個人的に大きいと思っています。Bookathonに参加することで、@<b>{読む側から書く側に変わるきっかけ}になります。技術書の執筆は敷居が高いと感じている人にとって、メンターのサポートを受けながら書き始められる環境は貴重です。
...
今回、仮説を立てて検証する重要性はよくわかりました。でも正直なところ、@<b>{まだ日常的にやろうとしても面倒臭さが勝ってしまう気がします。}Bookathonのような強制力がなくても続けられるようにするには、どうすればいいのでしょうか？そうならないために、以下のことを考えたいと思っています。

 * @<b>{問いを立てる→仮説を立てるというプロセスの手順を言語化する}：毎回ゼロから考えるのではなく、再現可能な手順にしておきたい
 * @<b>{最小限の仮説はどの程度か？を言語化する}：完璧な仮説を立てようとすると腰が重くなるので、「このくらいで十分」というラインを決めておきたい

//note[この節は現在思考中です]{
この節で書きたい内容は、まだ思考の途中です。Bookathon後に、実際に日常で仮説駆動学習を試してみた結果を踏まえて、加筆していく予定です。
//}
