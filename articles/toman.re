...
ここで、私と似た属性の人だけでなく、仕事や趣味でのエンジニアリング活動で何らかの課題を抱えている人にも、Bookathonは効果があるのではないかと考えました。私のやり方をそのまま真似するのではなく、各々が抱えている課題に対して、以下のような流れで取り組むのが理想的だと思っています。
...
この構造のポイントは、@<b>{自分で仮説を立てた上でメンターのアドバイスを聞く}ことです。仮説なしにアドバイスを聞くのと、仮説を持った上で聞くのとでは、得られる気づきの深さが違います。具体例として、次の節で私自身の考えがどう変わっていったかを紹介していきます。
...
イベント初日、メンターのwakameさんから、私の考え方を変えるアドバイスをいただきました。
...
慣れの問題に対しては、今回の経験がヒントになりました。今回の検証の中で、「型チェックがあるのに、なぜ型を深掘りするのか？」から問いが連鎖的に出てきた経験をしました。最初の一歩は、当たり前に使っているものを選んで「これって正確には何だっけ？」と聞くことでした。そこから先は自然と疑問が出てきます。

心理的な壁に対しても、今回の経験が答えをくれました。「このくらいシンプルなら大変じゃないのでは？」という仮説が外れたことで、response.json()のストリームの仕組みやSOLID原則の実感的理解に繋がりました。@<b>{間違った仮説こそ、学びが大きかった}のです。完璧な仮説を立てる必要はありません。「〜だと思う」と言えれば、それで十分です。

強制力の欠如に対しては、ログを残す仕組みを用意しておくのが有効だと考えています。ただし、Bookathonレベルの丁寧なログを毎日書くのは現実的ではありません。「〜だと思ったけど、〜だった」の一文だけでいい。タイムズチャンネルやObsidianなど、すでに使っている場所に書く。そして、「仮説を立てたとき」ではなく@<b>{「驚いたとき」をトリガーにする}。驚きは「自分の予想と現実がズレた瞬間」なので、それ自体が仮説検証の結果です。
